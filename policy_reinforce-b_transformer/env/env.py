...
class TSPEnv:
    ...
    def calc_distance_random_order(self):
        # 訪問順序をランダムに生成
        n_cities = len(self.coords)
        visit_orders = np.random.permutation(n_cities)
        visit_orders = np.append(visit_orders, visit_orders[0])  # 最初の都市に戻る

        # 訪問順序に基づく座標を取得
        ordered_coords = [self.coords[i] for i in visit_orders]
...
def main():
    ...
    random_distance = env.calc_distance_random_order()
    print(f'ランダム訪問距離: {random_distance}')
